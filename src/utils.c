...
//Makes a string out of the counter int, padding it to the right to make it BLOCKSIZE bytes long.
...
{
	char number[BLOCKSIZE];
	int num_digits = 0;
	//memset(string, 0, BLOCKSIZE);
	sprintf(number, "%lu", counter);

	// Read the counter from the number string generated by sprintf
	for (int i=0; i<BLOCKSIZE; i++)
	{
		if (number[i]!='\0')
		{
	...
	// Copy the digits from the number
    for (int j = 0; j < num_digits; j++) 
	{
        string[j] = number[j];
    }

	// Generate additional padding bytes based on the counter
    for (int k = num_digits; k < BLOCKSIZE; k++) 
	{
        string[k] = '=';
    }
}
...
//Creates BLOCKSIZE random bytes and uses them to populate the nonce array
